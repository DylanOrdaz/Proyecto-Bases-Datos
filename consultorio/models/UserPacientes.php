<?php
    ...
    class User implements IUser {
    	...
    	public function get(){
    		try{
                if(is_int($this->id)){
                    ...
                    $query = $this->con->prepare("SELECT paciente_id, consultorio_nombre,c.consultorio_id,paciente_nombre ,paciente_apellido,paciente_telefono,paciente_fecha_nacimiento from pacientes p INNER JOIN consultorios c ON p.consultorio_id = c.consultorio_id WHERE paciente_id = ?");
                    ...
                    $query = $this->con->prepare("SELECT paciente_id, consultorio_nombre, CONCAT(paciente_nombre,' ',paciente_apellido) AS Nombre,paciente_telefono,paciente_fecha_nacimiento from pacientes p INNER JOIN consultorios c ON p.consultorio_id = c.consultorio_id ORDER BY paciente_id");
        			$query->execute();
        			$this->con->close();
        ...
        public function calTime(){
    		try{
                if(is_int($this->id)){
                    $query = $this->con->prepare("SELECT getTimeLeft(?)");
                    $query->bindParam(1, $this->id, PDO::PARAM_INT);
        			$query->execute();
        			$this->con->close();
        			return $query->fetchAll(PDO::FETCH_OBJ);
